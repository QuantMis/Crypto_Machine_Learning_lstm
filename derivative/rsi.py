...
class Rsi(object):
    ...
    def calc_rsi(self, data):
        ...
        for idx in range(self.period, len(data)+1):
            # save for update function (update class params)
    ...
    def calc_rs(self):
        # calculate average to get rel strength
...
class StochRsi(Rsi):
    ...
    def calc_stoch_rsi(self):
        ...
        for idx in range(self.period*2-1, len(self.value)):
            window = self.value[idx-self.period:idx]
            high = np.amax(window)
            low = np.amin(window)
            stoch_rsi.append((self.value[idx-1])/(high - low))
    ...
    def update_stoch_hist(self, price):
        ...
        self.ma_k.append(np.average(self.ma_k[-self.smooth_d:]))
